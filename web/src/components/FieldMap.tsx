...
export function FieldMap({ field }: Props) {
  ...
  const activeRowId = field.rows.some((row) => row.id === selectedRowId) ? selectedRowId : field.rows[0]?.id;
  const slots = useMemo(() => activeRowId ? field.slots.filter((slot) => slot.rowId === activeRowId) : [], [activeRowId, field.slots]);
  const width = 720;
  const height = 330;
  return (
      ...
      <div className="field-map-stage">
        <svg viewBox={`0 0 ${width} ${height}`} role="group" aria-label={`${field.name} rows and slots`}>
          ...
          {field.rows.map((row, index) => {
            ...
            return <g key={row.id} className={`field-map-row${selected ? " is-selected" : ""}`} role="button" aria-pressed={selected} tabIndex={0} onClick={() => setSelectedRowId(row.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelectedRowId(row.id); }}><text x="24" y={y + 5}>{row.label}</text><line x1="110" x2="680" y1={y} y2={y} /></g>;
          })}
          {slots.map((slot) => {
            ...
            return <circle className="field-map-slot" key={slot.id} cx={x} cy={y} r="8"><title>{`${row?.label}, slot ${slot.order}`}</title></circle>;
          })}
        </svg>
        <div className="field-map-north"><span>N</span><i /></div>
      </div>
      <footer><span>Fixed 0° orientation</span><span>{activeRowId ? field.rows.find((row) => row.id === activeRowId)?.label : "All rows"}</span><span>{slots.length ? `${slots.length} visible slots` : "Select a row"}</span></footer>
    </section>
  );
}
