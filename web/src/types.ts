...
export interface ReservationMember {
  ...
  display_name: string;
  role: "owner" | "guest";
  status: "active" | "departing" | "released";
}
...
export interface PublicFieldDetail extends PublicField {
  allowedCrops: string[];
  rows: PublicFieldRow[];
  slots: PublicFieldSlot[];
...
export interface MarketDisplayProduct extends MarketProduct {
  cropRelation: string;
  effect: string;
  image: string;
  restock: string;
...
export interface SellGroup {
  ...
  cropType: string;
  itemId: string;
  tier: "poor" | "standard" | "fine" | "premium" | string;
  quantity: number;
  unitPrice: number;
  basePrice?: number;
  multiplier?: number;
...
export interface HubViewRequest {
  kind: string;
  route?: HubRoute;
  fieldId?: string;
...
export interface HubViewModel<T = unknown> {
...
export interface HubAdapter {
  ...
  load<T>(request: HubViewRequest): Promise<HubViewModel<T>>;
  dispatch(intent: Record<string, unknown>): Promise<IntentResult>;
  close(): Promise<void>;
}
