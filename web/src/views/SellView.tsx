...
export function SellView() {
  ...
  const aside = <div className="detail-inspector sell-inspector"><span className="inspector-kicker">Exact preview</span><Scales size={38} weight="thin" /><h2>{selectedGroups.length ? `${selectedGroups.length} produce lot${selectedGroups.length === 1 ? "" : "s"}` : "Nothing selected"}</h2><p>Base market price, plus your personal level bonus</p><div className="sell-bank-summary"><span>Personal bank</span><strong>{money(data.bankBalance)}</strong><small>{selectedGroups.length ? `After sale ${money(data.bankBalance + total)}` : "Credit is immediate after confirmation"}</small></div><div className="inspector-rule" />{selectedGroups.length ? <div className="sell-preview-lines">{selectedGroups.map((group) => <div key={group.key}><span>{selected[group.key]}× {cropLabel(group.cropType)}</span><strong>{money(lineTotal(group, selected[group.key], data.sellBonus))}</strong></div>)}</div> : <p className="inspector-description">Select quantities from your eligible harvest. Only produce created by this resource and harvested by you appears here.</p>}<div className="supply-cart-total"><span>Bank credit</span><strong>{money(total)}</strong></div><div className="physical-note">{hub.surface === "sell" ? <CheckCircle size={19} /> : <MapPin size={19} />}<span>{hub.surface === "sell" ? "You are present at the authorized Grapeseed buyer." : "Remote tablet is preview-only. Mark the buyer route to sell."}</span></div>{hub.surface === "sell" ? <button className="inspector-action" type="button" disabled={!selectedGroups.length || busy} onClick={() => setConfirmOpen(true)}>Review Sale<CaretRight size={18} /></button> : <button className="inspector-action" type="button" onClick={() => void setRoute()}>Route to Buyer<MapPin size={18} /></button>}</div>;

  return <HubScaffold eyebrow="Personal produce buyer" title="Sell" subtitle="Sell only your own verified harvest at direct market prices." toolbar={toolbar} aside={aside}>
