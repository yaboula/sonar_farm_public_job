...
export function useHubView<T>(request: HubViewRequest) {
  const { adapter, revision } = useHub(); const [state, setState] = useState<ViewState>("loading");
  ...
  useEffect(() => { void Promise.resolve().then(reload); }, [reload, revision]); return { state, data, error, reload };
}
