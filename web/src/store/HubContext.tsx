...
const HubContext = createContext<(HubContextModel & { adapter: typeof hubAdapter; revision: number }) | null>(null);
export function HubProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "hub:invalidate") setRevision((value) => value + 1);
    };
  ...
  return <HubContext.Provider value={{ ...model, adapter: hubAdapter, revision }}>{children}</HubContext.Provider>;
}
