...
`Config.Gameplay.ActionFeedback` defines only a target duration and a pure animation-dictionary clip for Plant, Water, Fertilize, Weed, Treat pests and Harvest. Farming actions intentionally create no handheld props, audio emitters, particles or GTA scenarios. `client/modules/interaction/feedback.lua` owns the `prepare -> perform -> finish/cancel` lifecycle and guarantees animation cleanup on cancellation, death, excess distance, vehicle entry, job/duty loss, presentation errors and resource stop.

The runtime reads the native GTA clip duration with `GetAnimDuration`, selects the closest whole-cycle duration to the configured target, and sends that exact effective duration to both `TaskPlayAnim` and the progress circle. Before playback, the same action mutex walks the player to the selected streamed Slot's configured stand-off point and turns the ped toward the real crop coordinate. The animation never starts if that point cannot be reached or the player becomes ineligible. The mutex remains held through the authoritative server callback, so another farming action or the Hub cannot start in between. Use `/sfpj_action_preview <action>` only with `Config.Debug` and the configured admin ACE to approve clip posture and timing on freemode male and female characters.
